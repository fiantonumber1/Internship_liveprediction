...
def audio_samples(in_data, frame_count, time_info, status_flags):
    ...
    with graph.as_default():
        if x.shape[0] != 0:
            ...
            pred = load_model(model_filename).predict(x)
        ...
        for prediction in predictions:
            ...
            if (m < len(label)):
                p = label[m]
                suarane=("Prediction: %s (%0.2f)" % (ubicoustics.to_human_labels[label[m]], prediction[0,m]))
                coughing=suarane[0:20]
                if coughing =="Prediction: Coughing":
                    print("Prediction: %s (%0.2f)" % (ubicoustics.to_human_labels[label[m]], prediction[0,m]))
                else:
                    print(coughing,"Not coughing sound")
